...
```{r loan and clean 2020 census data, results = "hide"}
...
flood_train_df$flood1 <- as.numeric(flood_train_df$flood) - 1
plot2 <- flood_train_df %>%
   select(flood1, white, male_by_age_total, hhincome, employment_rate) %>%
  na.omit()
round(cor(plot2), 1)
...
P3 <- ggplot() + 
  geom_sf_interactive(
    ...
    color = "white",
    aes(fill = hhincome, tooltip = tooltip, data_id = hhincome),
    size = 0.2, 
    ) + 
  geom_sf(
    data = filter(flood_train_sf_plot, flood == 1), 
    fill = "red", 
    color = "red", 
    alpha = 0.5
    ) +
  ...
  labs(
    title = "Median Household Income in New York City",
...
#  scale_fill_distiller(palette = "Blues", direction = 1) + 
  labs(
    title = "Rate of Minorities in New York City",
