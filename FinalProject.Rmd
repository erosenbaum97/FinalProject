...
flood_demography_sf <- demography_sf %>%
  ...
  subset(select = -c(geoid, census_tract, county)) %>% 
  na.omit() %>% 
...
flood_train_df <- flood_train_sf %>% 
  st_centroid() %>% 
  extract(geometry, into = c('Lat', 'Lon'), '\\((.*),(.*)\\)', remove = TRUE) %>% 
  select(-geometry) %>%
...
predictions <- bind_cols(
  flood_test_df,
  predict(object = cart_fit, new_data = flood_test_df),
  predict(object = cart_fit, new_data = flood_test_df, type = "prob")
)
...
predictionslog <- bind_cols(
  flood_test_df,
  predict(object = logistic_fit, new_data = flood_test_df),
  predict(object = logistic_fit, new_data = flood_test_df, type = "prob")
)
...
predictionsknn <- bind_cols(
  flood_test_df,
  predict(object = knn_fit, new_data = flood_test_df)
)
