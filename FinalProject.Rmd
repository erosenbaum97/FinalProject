...
Sandy_folds <- vfold_cv(tibble(Sandy_train)[4:19], v = 10, repeats = 1)

Sandy_rec <- recipe(total_dmg ~ ., data = tibble(Sandy_train)[4:19]) %>% 
  step_center(all_predictors()) %>% 
  step_scale(all_predictors())
...
rf_grid<- Sandy_rf_mod %>% 
  parameters() %>% 
  finalize(tibble(Sandy_train)[5:19]) %>% 
...
bind_cols(
  ...
  ggplot() +
  geom_point(
    aes(x = fold, y = rmse, color = model)
  )

## average RMSE of lasso model
Sandy_rf_rmse %>%
  summarise(
    mean = mean(Sandy_rf_rmse)
  )

## average RMSE of random forest model
Sandy_en_rmse %>%
  summarise(
    mean = mean(Sandy_en_rmse)
  )

## average RMSE of decision tree model
Sandy_knn_rmse %>%
  summarise(
    mean = mean(Sandy_knn_rmse)
  )
...
Sandy_final_fit <- Sandy_final_wf %>%
  fit(data = tibble(Sandy_train)[3:19])

bind_cols(
  Sandy_test,
  predict_dmg = predict(object = Sandy_final_fit, new_data = tibble(Sandy_test)[3:19])
) %>%
  rmse(truth = total_dmg, estimate = .pred)
